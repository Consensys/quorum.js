...
module.exports = (web3, enclaveOptions) => {
  const enclave = GenericEnclave(web3, enclaveOptions.ipcPath, enclaveOptions.publicUrl, enclaveOptions.privateUrl);
  ...
  const encryptRawTransactionViaSendAPI = ({ data, privateFrom, privateFor }) => {
    return enclave
      .storeRawRequestViaSendAPI(hexToBase64(data.substring(2)), privateFrom, privateFor)
      .then(response => {
        return base64toHex(response.key);
      });
  };
  ...
  const sendRawTransactionViaSendAPI = options => {
    return getTransactionPayloadViaSendAPI(options).then(transactionPayload => {
      ...
      return web3.eth.sendSignedTransaction(`0x${privateTx.toString("hex")}`, options.privateFor).then(result => {
        return result;
      });
