...
module.exports = (web3, enclaveOptions) => {
  ...
  const sendRawRequest = (payload, privateFor, privacyFlag = undefined) => {
    return enclave.sendRawRequest(payload, privateFor, privacyFlag);
  };
   ...
   * @param {String} options.privateFrom
   * @param {Array<String>} options.privateFor    Array of public keys to whom this tx is privateFor.
   * @param {int} options.privacyFlag             0/undefined - StnadardPrivate, 1 - PartyProtection, 3 - PrivateStateValidation
   * @param {Object} options.nonce
   * @param {Object} options.privacyFlag          The privacy flag: 0 - StandardPrivate, 1 - Party Protection, 3 - PSV
   * @returns {Promise}                   resolves if the raw transaction was sent successfully else rejects with error.
   */
  const sendRawTransaction = options => {
    return getTransactionPayload(options).then(transactionPayload => {
      ...
      // eslint-disable-next-line promise/no-nesting
      return enclave
        .sendRawRequest(`0x${privateTx.toString("hex")}`, options.privateFor, options.privacyFlag)
        .then(result => {
          return result;
        });
    });
